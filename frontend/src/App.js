...
class App extends Component {
  ...
  refreshList = () => {
    axios //Axios to send and receive HTTP requests
      .get(`${axios.defaults.baseURL}/api/tasks/`)
  ...
  handleSubmit = (item) => {
    ...
    if (item.id) {
      ...
      axios
        .put(`${axios.defaults.baseURL}/api/tasks/${item.id}/`, item)
        .then((res) => this.refreshList());
      return;
    }
    // if new post to submit
    axios
      .post(`${axios.defaults.baseURL}/api/tasks/`, item)
      .then((res) => this.refreshList());
  };
  ...
  handleDelete = (item) => {
    axios
      .delete(`${axios.defaults.baseURL}/api/tasks/${item.id}/`)
      .then((res) => this.refreshList());
  };
  ...
  render() {
    return (
      <main className="content p-3 mb-2 bg-info ">
        <h1 className="text-white text-center my-4">My To-Do List</h1>
        <div className="row ">
